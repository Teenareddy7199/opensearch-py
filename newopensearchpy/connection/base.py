...
# create the newopensearchpy.trace logger, but only set propagate to False if the
# logger hasn't already been configured
_tracer_already_configured = (
    "newopensearchpy.trace" in logging.Logger.manager.loggerDict
)
tracer = logging.getLogger("newopensearchpy.trace")
...
class Connection(object):
    ...
    def _get_default_user_agent(self):
        return "opensearch_py_new/%s (Python %s)" % (__versionstr__, python_version())
